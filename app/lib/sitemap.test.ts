import sitemap, { SitemapOptions } from "./sitemap"; // サイトマップジェネレーターのパスを適切に調整してください
...
test("sitemap generator creates valid XML", async () => {
...
test("sitemap generator respects exclude option", async () => {
...
test("sitemap generator uses custom frequency and priority", async () => {
...
test("sitemap generator throws error for invalid priority", async () => {
  const app = createMockApp(["/", "/about"]);
  const options: SitemapOptions = {
    app,
    hostname: "https://example.com",
...
test("sitemap generator throws error for invalid frequency", async () => {
  const app = createMockApp(["/", "/about"]);
  const options: SitemapOptions = {
    app,
    hostname: "https://example.com",
...
test("sitemap generator uses default values when not provided", async () => {
  const app = createMockApp(["/", "/about"]);
  const options: SitemapOptions = {
    app,
  };
